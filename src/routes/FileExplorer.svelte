<script>
  import { invoke } from "@tauri-apps/api/core";
    ...
    async function open_media() {
        path.push(await invoke("select_media"));
...
<div id="file_explorer">
    <div id="buttons">
        <button><Svg name="file_open" color="rgb(186, 197, 211)"/><span>Add File</span></button>
        <button onclick={open_media}><Svg name="folder_open" color="rgb(186, 197, 211)"/><span>Add Folder</span></button>
    </div>
    <div id="added_paths">
    {#each path as pathItem}
        <p>{pathItem}</p>
    {/each}
    </div>
